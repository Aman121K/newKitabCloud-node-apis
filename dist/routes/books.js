...
router.get('/books/:id', requireJwt, async (req, res) => {
    try {
        ...
        const [bookRows] = await pool.query('SELECT * FROM books WHERE id = ? LIMIT 1', [id]);
        if (!Array.isArray(bookRows) || !bookRows[0])
            return res.status(404).json({ status: 'true', data: null });
        ...
        const [reviews] = await pool.query(`SELECT r.*, u.id as user_id, u.full_name, u.email
			 FROM reviews r LEFT JOIN users u ON u.id = r.user_id
			 WHERE r.reviewable_type = 'App\\\\Models\\\\Book' AND r.reviewable_id = ?`, [id]);
        return res.json({
            status: 'true',
            data: { ...book, author: author[0] || null, reader: reader[0] || null, publisher: publisher[0] || null, category: category[0] || null, language: language[0] || null, review: reviews, tags }
        });
    }
    catch (e) {
...
router.post('/review', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const { type, review, emoji, rating, bookid, readerid, authorid, publisherid, average_rating } = req.body || {};
        let reviewableType = '', reviewableId;
        if (type === 'book') {
            reviewableType = 'App\\\\Models\\\\Book';
            reviewableId = bookid;
        }
        if (type === 'reader') {
            reviewableType = 'App\\\\Models\\\\Reader';
            reviewableId = readerid;
        }
        if (type === 'author') {
            reviewableType = 'App\\\\Models\\\\Author';
            reviewableId = authorid;
        }
        if (type === 'publisher') {
            reviewableType = 'App\\\\Models\\\\Publisher';
            reviewableId = publisherid;
        }
        if (!reviewableType || !reviewableId)
            return res.status(400).json({ status: false, message: 'Invalid review target' });
        await pool.execute(`INSERT INTO reviews (rating, reviewable_id, reviewable_type, user_id, review, emoji, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`, [rating || 0, reviewableId, reviewableType, id, review || null, emoji ? JSON.stringify(emoji) : null]);
        if (type === 'book' && average_rating)
            await pool.execute(`UPDATE books SET average_rating = ? WHERE id = ?`, [average_rating, bookid]);
...
router.post('/book_played', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const { author_id, reader_id, publisher_id, book_id, category_id } = req.body || {};
        await pool.execute(`INSERT INTO book_playeds (author_id, reader_id, publisher_id, book_id, category_id, user_id, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, NOW(), NOW())`, [author_id || null, reader_id || null, publisher_id || null, book_id || null, category_id || null, id]);
...
router.get('/book_played', requireJwt, async (_req, res) => {
    try {
        const [rows] = await pool.query(`SELECT b.*, bp.user_id, bp.created_at as played_at
...
router.get('/get_featured_books', requireJwt, async (_req, res) => {
    try {
        ...
        return res.status(500).json({ ststus: false, message: e?.message || 'Internal error' });
    }
});
router.get('/booklike/:bookid', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const bookId = parseInt(req.params.bookid, 10);
        const [rows] = await pool.query(`SELECT id FROM book_likes WHERE user_id = ? AND book_id = ? LIMIT 1`, [id, bookId]);
...
router.get('/bookmark/:bookid', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const bookId = parseInt(req.params.bookid, 10);
        const [rows] = await pool.query(`SELECT id FROM bookmark_models WHERE user_id = ? AND book_id = ? LIMIT 1`, [id, bookId]);
        if (Array.isArray(rows) && rows[0]) {
...
router.get('/getLikedBooks', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const [rows] = await pool.query(`SELECT b.* FROM books b JOIN book_likes bl ON bl.book_id = b.id WHERE bl.user_id = ?`, [id]);
        return res.json({ status: 'true', data: rows });
    }
    catch (e) {
        return res.status(500).json({ status: 'false', message: e?.message || 'Internal error' });
...
router.get('/getBookmarkBooks', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const [rows] = await pool.query(`SELECT b.* FROM books b JOIN bookmark_models bm ON bm.book_id = b.id WHERE bm.user_id = ?`, [id]);
        return res.json({ status: 'true', data: rows });
    }
    catch (e) {
        return res.status(500).json({ status: 'false', message: e?.message || 'Internal error' });
