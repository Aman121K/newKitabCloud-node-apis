// node-api/src/routes/readers.ts
...
router.get('/:id', requireJwt, async (req, res) => {
  try {
    ...
    const [rows]: any = await pool.query(
      'SELECT id, name, email, phone, about, facebook, instagram, snapchat, twitter, youtube, image, status, created_at, updated_at FROM readers WHERE id = ? AND status = 1', 
      [id]
    );
...
router.post('/', requireJwt, async (req, res) => {
  try {
    ...
    const [existing]: any = await pool.query('SELECT id FROM readers WHERE email = ?', [email]);
    ...
    await pool.execute(
      'INSERT INTO readers (name, email, password, phone, about, facebook, instagram, snapchat, twitter, youtube, status, created_at, updated_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 1, NOW(), NOW())',
      [name, email, hashedPassword, phone, about, facebook, instagram, snapchat, twitter, youtube]
    );
    
    res.json({ success: true, message: 'Reader created successfully' });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.put('/:id', requireJwt, async (req, res) => {
  try {
    ...
    const [existing]: any = await pool.query('SELECT id FROM readers WHERE email = ? AND id != ?', [email, id]);
    if (existing.length > 0) {
      return res.status(400).json({ success: false, message: 'Email already exists' });
    }
    
    await pool.execute(
      'UPDATE readers SET name = ?, email = ?, phone = ?, about = ?, facebook = ?, instagram = ?, snapchat = ?, twitter = ?, youtube = ?, updated_at = NOW() WHERE id = ?',
      [name, email, phone, about, facebook, instagram, snapchat, twitter, youtube, id]
    );
    
    res.json({ success: true, message: 'Reader updated successfully' });
  } catch (error: any) {
    res.status(500).json({ success: false, message: error.message });
  }
});
...
router.delete('/:id', requireJwt, async (req, res) => {
  try {
    ...
    // Check if reader is being used by books
    const [books]: any = await pool.query('SELECT COUNT(*) as count FROM books WHERE reader_id = ?', [id]);
    if (books[0].count > 0) {
      return res.status(400).json({ 
        success: false, 
        message: 'Cannot delete reader that is being used by books' 
      });
    }
