...
router.post('/follower', requireJwt, async (req: any, res) => {
	try {
		const id = req.user?.id || req.user?.sub;
		const { type, author_id, reader_id, publisher_id } = req.body || {};
		if (type === 'author') {
			const [rows]: any = await pool.query(`SELECT id FROM author_followers WHERE author_id = ? AND user_id = ? LIMIT 1`, [author_id, id]);
			if (Array.isArray(rows) && rows[0]) { await pool.execute(`DELETE FROM author_followers WHERE id = ?`, [rows[0].id]); return res.json({ status: true, message: 'Unfollow Author' }); }
		...
		if (type === 'reader') {
			const [rows]: any = await pool.query(`SELECT id FROM reader_followers WHERE reader_id = ? AND user_id = ? LIMIT 1`, [reader_id, id]);
			if (Array.isArray(rows) && rows[0]) { await pool.execute(`DELETE FROM reader_followers WHERE id = ?`, [rows[0].id]); return res.json({ status: true, message: 'Unfollow Reader' }); }
		...
		if (type === 'publisher') {
			const [rows]: any = await pool.query(`SELECT id FROM publisher_followers WHERE publisher_id = ? AND user_id = ? LIMIT 1`, [publisher_id, id]);
			if (Array.isArray(rows) && rows[0]) { await pool.execute(`DELETE FROM publisher_followers WHERE id = ?`, [rows[0].id]); return res.json({ status: true, message: 'Unfollow Publisher' }); }
