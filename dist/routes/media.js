...
router.get('/audio', requireJwt, async (_req, res) => {
    try {
        const [cats] = await pool.query(`SELECT c.* FROM categories c WHERE EXISTS (SELECT 1 FROM books b WHERE b.category_id = c.id AND b.bookaudio IS NOT NULL AND b.bookaudio <> '')`);
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [books] = await pool.query(`SELECT * FROM books WHERE category_id = ? AND bookaudio IS NOT NULL AND bookaudio <> '' ORDER BY RAND() LIMIT 5`, [cat.id]);
            cat.books = books;
        }
...
router.get('/ebooks', requireJwt, async (_req, res) => {
    try {
        const [cats] = await pool.query(`SELECT c.* FROM categories c WHERE EXISTS (SELECT 1 FROM books b WHERE b.category_id = c.id AND b.bookfile IS NOT NULL AND b.bookfile <> '')`);
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [books] = await pool.query(`SELECT * FROM books WHERE category_id = ? AND bookfile IS NOT NULL AND bookfile <> '' ORDER BY RAND() LIMIT 5`, [cat.id]);
            cat.books = books;
        }
...
router.get('/magazines', requireJwt, async (_req, res) => {
    try {
        ...
        const out = [];
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [books] = await pool.query(`SELECT * FROM books WHERE category_id = ? AND is_magazine = 1 ORDER BY RAND() LIMIT 5`, [cat.id]);
...
router.get('/podcast', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const [cats] = await pool.query(`SELECT c.* FROM categories c JOIN user_categories uc ON uc.user_category_id = c.id WHERE uc.user_id = ?`, [id]);
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [podcasts] = await pool.query(`SELECT * FROM podcasts WHERE category_id = ? ORDER BY RAND() LIMIT 5`, [cat.id]);
...
router.get('/videos', requireJwt, async (_req, res) => {
    try {
        ...
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [videos] = await pool.query(`SELECT * FROM videos WHERE category_id = ? ORDER BY RAND() LIMIT 5`, [cat.id]);
            cat.videos = videos;
        }
...
router.get('/getAudiobooksByLanguage', requireJwt, async (req, res) => {
    try {
        ...
        const [cats] = await pool.query(`SELECT * FROM categories`);
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [books] = await pool.query(`SELECT * FROM books WHERE category_id = ? AND language_id = ? AND bookaudio IS NOT NULL AND bookaudio <> '' ORDER BY RAND() LIMIT 5`, [cat.id, languageId]);
            cat.books = books;
        }
        return res.json({ categoryWithAudioBooks: cats });
    }
...
router.get('/getEbooksByLanguage', requireJwt, async (req, res) => {
    try {
        ...
        const [cats] = await pool.query(`SELECT * FROM categories`);
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [books] = await pool.query(`SELECT * FROM books WHERE category_id = ? AND language_id = ? AND bookfile IS NOT NULL AND bookfile <> '' ORDER BY RAND() LIMIT 5`, [cat.id, languageId]);
            cat.books = books;
        }
        return res.json({ categoryWithEBooks: cats });
    }
...
router.get('/getMagazinesByLanguage', requireJwt, async (req, res) => {
    try {
        ...
        const out = [];
        for (const cat of Array.isArray(cats) ? cats : []) {
            const [books] = await pool.query(`SELECT * FROM books WHERE category_id = ? AND language_id = ? AND is_magazine = 1 ORDER BY RAND() LIMIT 5`, [cat.id, languageId]);
            if (Array.isArray(books) && books.length)
                out.push({ ...cat, books });
        }
        return res.json({ categoryWithMagazines: out });
...
router.get('/getBooksByLanguageCategory', requireJwt, async (req, res) => {
    try {
        const languageId = req.query.language_id ? parseInt(req.query.language_id, 10) : undefined;
...
router.get('/videoLike/:video_id', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const videoId = parseInt(req.params.video_id, 10);
        const likeableType = 'App\\\\Models\\\\Video';
        const [rows] = await pool.query(`SELECT id FROM likes WHERE user_id = ? AND likeable_id = ? AND likeable_type = ? LIMIT 1`, [id, videoId, likeableType]);
        ...
        await pool.execute(`INSERT INTO likes (user_id, likeable_id, likeable_type, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())`, [id, videoId, likeableType]);
...
router.get('/podcastLike/:podcast_id', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const podcastId = parseInt(req.params.podcast_id, 10);
        const likeableType = 'App\\\\Models\\\\Podcast';
        const [rows] = await pool.query(`SELECT id FROM likes WHERE user_id = ? AND likeable_id = ? AND likeable_type = ? LIMIT 1`, [id, podcastId, likeableType]);
        if (Array.isArray(rows) && rows[0]) {
            await pool.execute(`DELETE FROM likes WHERE id = ?`, [rows[0].id]);
            return res.json({ status: true, message: 'Podcast Unliked Successfully' });
        }
        await pool.execute(`INSERT INTO likes (user_id, likeable_id, likeable_type, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())`, [id, podcastId, likeableType]);
...
router.get('/videoPlayed/:video_id', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const videoId = parseInt(req.params.video_id, 10);
        ...
        await pool.execute(`INSERT INTO playeds (user_id, played_id, played_type, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())`, [id, videoId, playedType]);
...
router.get('/podcastPlayed/:podcast_id', requireJwt, async (req, res) => {
    try {
        const id = req.user?.id || req.user?.sub;
        const podcastId = parseInt(req.params.podcast_id, 10);
        ...
        await pool.execute(`INSERT INTO playeds (user_id, played_id, played_type, created_at, updated_at) VALUES (?, ?, ?, NOW(), NOW())`, [id, podcastId, playedType]);
        return res.json({ status: true, message: 'Count Added Successfully' });
    }
    catch (e) {
        return res.status(500).json({ status: false, message: e?.message || 'Internal error' });
