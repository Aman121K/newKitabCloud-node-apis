...
router.get('/home_data', requireJwt, async (req: any, res) => {
	try {
		...
		for (const cat of Array.isArray(cats) ? (cats as any[]) : []) {
			const [books] = await pool.query(`SELECT b.* FROM books b WHERE b.category_id = ? ORDER BY RAND() LIMIT 5`, [(cat as any).id]);
			(cat as any).books = books;
		}
		const [bookmark] = await pool.query(
			`SELECT bm.*, b.* FROM bookmark_models bm JOIN books b ON b.id = bm.book_id WHERE bm.user_id = ?`, [userId]
		);
	...
	try {
		...
		for (const cat of Array.isArray(cats) ? (cats as any[]) : []) {
			const [books] = await pool.query(
				`SELECT b.* FROM books b WHERE b.category_id = ? AND b.language_id = ? ORDER BY RAND() LIMIT 5`,
...
router.post('/clickads', requireJwt, async (req: any, res) => {
	try {
		const { ad_id } = req.body;
		if (!ad_id) return res.status(400).json({ status: false, message: 'ad_id required' });
		
		// Increment click count for advertisement
		await pool.execute('UPDATE advertisements SET click_count = COALESCE(click_count, 0) + 1 WHERE id = ?', [ad_id]);
		
		return res.json({ status: true, message: 'Ad click recorded successfully' });
