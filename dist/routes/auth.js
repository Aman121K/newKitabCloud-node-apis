...
router.post('/register', async (req, res) => {
    try {
        ...
        if (existing)
            return res.status(200).json({ error: 'The email has already been taken.' });
...
router.post('/login', async (req, res) => {
    try {
        ...
        if (!email || !password)
            return res.status(200).json({ error: { email: ['email is required'], password: ['password is required'] } });
        const user = await getUserByEmail(email);
        if (!user)
            return res.status(200).json({ success: false, message: 'Login credentials are invalid.' });
...
router.post('/socialSignup', async (req, res) => {
    try {
        ...
        const [rows] = await pool.query('SELECT * FROM users WHERE provider_id = ? LIMIT 1', [provider_id]);
        ...
        const [result] = await pool.execute(`INSERT INTO users (full_name, email, country, provider_id, password, device_type, device_token, fcm_token, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [full_name || '', email || '', country || null, provider_id, hash, device_type || null, device_token || null, fcm_token || null]);
...
router.post('/socialLogin', async (req, res) => {
    try {
        ...
        if (!(Array.isArray(rows) && rows[0])) {
            ...
            const hash = await bcrypt.hash(password, 10);
            await pool.execute(`INSERT INTO users (full_name, email, country, provider_id, password, device_type, device_token, fcm_token, created_at, updated_at)
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [full_name || '', email || '', country || null, provider_id, hash, device_type || null, device_token || null, fcm_token || null]);
        }
        const [rows2] = await pool.query('SELECT * FROM users WHERE provider_id = ? LIMIT 1', [provider_id]);
        const u = rows2[0];
        const ok = await bcrypt.compare(password, u.password);
        if (!ok)
            return res.status(400).json({ success: false, message: 'Could not create token.' });
        const token = signToken({ id: u.id, email: u.email });
        return res.json({ success: true, token, user: u });
    }
    catch (e) {
...
router.post('/appleloginSignup', async (req, res) => {
    try {
        ...
        const [exists] = await pool.query('SELECT * FROM users WHERE provider_id = ? LIMIT 1', [provider_id]);
        ...
        const hash = await bcrypt.hash(password, 10);
        const [ins] = await pool.execute(`INSERT INTO users (full_name, email, country, provider_id, password, device_type, device_token, fcm_token, created_at, updated_at)
			 VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())`, [full_name || '', email || '', country || null, provider_id, hash, device_type || null, device_token || null, fcm_token || null]);
...
router.post('/forgot', async (req, res) => {
    try {
        ...
        if (!email)
            return res.status(422).json({ status: false, message: 'Validation errors', errors: { email: ['email is required'] } });
        const user = await getUserByEmail(email);
        if (!user)
...
router.post('/verify-otp', async (req, res) => {
    try {
        ...
        if (!otp)
            return res.status(422).json({ status: false, message: 'Validation errors', errors: { otp: ['otp is required'] } });
        const ok = await verifyOtpCode(email, otp);
        if (!ok.ok)
            return res.json({ status: false, message: ok.message || 'Invalid OTP.' });
...
router.post('/reset-password', async (req, res) => {
    try {
        ...
        if (!password || password !== password_confirmation)
            return res.status(422).json({ status: false, message: 'Validation errors', errors: { password: ['password confirmation mismatch'] } });
